...
error_output_pattern = re.compile(r'\[Required output\]\s+(.*)\[Your output\]', re.DOTALL)
...
def get_error_output(error_log):
    match = error_output_pattern.search(error_log)
    if match:
        return match.group(1)
